...
namespace Smartstore.Core.Logging
{
    internal class RequestLoggingMiddleware
    {
        ...
        bool LogCompletion(HttpContext httpContext, DiagnosticContextCollector collector, int statusCode, double elapsedMs, Exception ex)
        {
            ...
            if (ex != null)
            {
                var message = GetMessage(httpContext, ex);
                logger.Write(level, ex, message);

                if (level == LogEventLevel.Error)
                {
                    SlackLogger.LogMessageToSlack(Microsoft.Extensions.Logging.LogLevel.Error, ex, message);
                }
                if (level == LogEventLevel.Fatal)
                {
                    SlackLogger.LogMessageToSlack(Microsoft.Extensions.Logging.LogLevel.Critical, ex, message);
                }
            }
            else
            {
