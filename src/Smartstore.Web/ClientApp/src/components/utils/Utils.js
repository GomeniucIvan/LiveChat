...
export function toBoolean(value) {
    if (typeof value === 'boolean') {
        ...
        // If the value is a string, convert it to lower case and check for truthy values.
        const lowerCaseValue = value.toLowerCase();
        return lowerCaseValue === 'true' || lowerCaseValue === 'yes' || lowerCaseValue === '1';
    } else {
        // For all other values, attempt to convert them to a boolean using JavaScript's built-in Boolean function.
        return Boolean(value);
    }
}

export function redirectToLogin(location, response, forceRedirect) {
    if (!isNullOrEmpty(location) && !isNullOrEmpty(location.pathname)) {
        let redirectToLoginPage = false;

        if (forceRedirect) {
            if (location.pathname.toLocaleLowerCase() !== '/install') {
                redirectToLoginPage = true;
            }
        }

        if (!isNullOrEmpty(response)) {
            var forceLogOut = response.ForceLogOut;
            if (toBoolean(forceLogOut)) {
                redirectToLoginPage = true;
            }
        }
